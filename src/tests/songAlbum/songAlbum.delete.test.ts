...
describe("testes unitários do método delete do service de songAlbum", () => {
  ...
  },
  {
    ...
    mockRepository.delete.mockImplementation(async (songId, albumId) => {
        const indexItem = songAlbumDB.findIndex(item => item.album_id === albumId && item.song_id === songId)
        if(indexItem === -1) throw new NotFoundError('relação não existente')
        songAlbumDB.splice(indexItem, 1)
    })
  });

  it("success case: deve deletar a relação de acordo com o songId e albumId", async () => {
    const deletedData = {...songAlbumDB[0]}
    const songAlbum: ISongAlbum = { song_id: 1, album_id: 1 };
    await service.delete(songAlbum.song_id, songAlbum.album_id);
    console.log(
  ...
  it("error case: deve dar erro pois o song id nao foi passado", async () => {
    try {
      ...
      await service.delete(songAlbum.songId, songAlbum.albumId);
